...
recent_planes = {}  # Dictionary to store recently seen planes  and their timestamps, so i don't double notify if they are traveling slow
...
def extract_flight_information(flight_icao):
   ...
   # Extract the desired information
    if json_data['identification']['number'] is not None:
        flight_number = json_data['identification']['number'].get('default', 'No Data Available')
        returnData.update({"Flight Number": flight_number})
    if  json_data['airport']['origin'] is not None:       
        departure_airport = json_data['airport']['origin'].get('name', 'No Data Available')
        if json_data['airport']['origin']['code'] is not None:    
            departure_iata_code = json_data['airport']['origin']['code'].get('iata', 'No Data Available')
            returnData.update({"Departure Airport": f"{departure_airport} ({departure_iata_code})"})
    if json_data['airport']['destination'] is not None:       
        arrival_airport = json_data['airport']['destination'].get('name', 'No Data Available')
        if json_data['airport']['destination']['code'] is not None:       
            arrival_iata_code = json_data['airport']['destination']['code'].get('iata', 'No Data Available')
            returnData.update({"Arrival Airport": f"{arrival_airport} ({arrival_iata_code})"})
    ...
    try: 
        ...
        for flight_key, flight_data in json_data.items():
            if isinstance(flight_data, list):
                ...
                if not haveSeenRecently(flight_icao):
                    ...
                    flight_information = extract_flight_information(flight_icao)
                    ...
                    if data['Flight Number'] is not None:
                        ...
                        mqttClient.publish(topic, json_message)
...
def haveSeenRecently(flight_icao):
    ...
    try:
        expired_ids = [key for key, value in recent_planes.items() if current_time - value >= 60]
